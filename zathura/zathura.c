  ...
  if (document_info->zathura != NULL && document_info->path != NULL) {
    ...
    if (file != NULL) {
      if (document_info->synctex != NULL) {
        ...
        document_open(document_info->zathura, file, uri, document_info->password, document_info->page_number);
      }
      g_free(file);
      g_free(uri);
...
static void
document_open_page_most_frequent_size(zathura_document_t* document, unsigned int* width, unsigned int* height)
{
  ...
  for (unsigned int page_id = 0; page_id < number_of_pages; ++page_id) {
    ...
    GIRARA_LIST_FOREACH_BODY(
      samples, sample_t*, sample, if (fabs(sample->h - h) <= DBL_EPSILON && fabs(sample->w - w) <= DBL_EPSILON) {
        sample->freq++;
        found = true;
        break;
      });
...
bool
document_open(zathura_t* zathura, const char* path, const char* uri, const char* password, int page_number)
{
...
bool
document_open_synctex(zathura_t* zathura, const char* path, const char* uri, const char* password, const char* synctex)
{
  bool ret = document_open(zathura, path, password, uri, ZATHURA_PAGE_NUMBER_UNSPECIFIED);
  if (ret == false) {
    return false;
  }
  if (synctex == NULL) {
...
void
document_open_idle(zathura_t* zathura, const char* path, const char* password, int page_number, const char* mode,
...
save_fileinfo_to_db(zathura_t* zathura)
{
  ...
  zathura_fileinfo_t file_info = {.current_page           = zathura_document_get_current_page_number(zathura->document),
...
void
page_widget_set_mode(zathura_t* zathura, unsigned int page_padding, unsigned int pages_per_row,
